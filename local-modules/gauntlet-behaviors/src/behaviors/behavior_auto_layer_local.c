/*
 * Local copy of urob's auto-layer behavior for Gauntlet-specific extensions.
 * We keep max-keypress support here so the vendored zmk-auto-layer checkout can
 * stay unmodified and easy to update from upstream.
 *
 * Original work Copyright (c) 2021 The ZMK Contributors
 *
 * SPDX-License-Identifier: MIT
 */

#define DT_DRV_COMPAT zmk_behavior_auto_layer_local
...
#define ZMK_BHV_AUTO_LAYER_LOCAL_MAX_ACTIVE 10
...
struct behavior_auto_layer_local_config {
    const struct key_list *continue_keys;
    bool ignore_alphas;
    bool ignore_numbers;
    bool ignore_modifiers;
    uint16_t max_keypresses;
};

struct active_auto_layer_local {
    bool is_active;
    uint8_t layer;
    const struct behavior_auto_layer_local_config *config;
    uint16_t keypresses_remaining;
};

static struct active_auto_layer_local active_auto_layers[ZMK_BHV_AUTO_LAYER_LOCAL_MAX_ACTIVE] = {};

static struct active_auto_layer_local *find_auto_layers(uint8_t layer) {
    for (int i = 0; i < ZMK_BHV_AUTO_LAYER_LOCAL_MAX_ACTIVE; i++) {
...
static int new_auto_layer(uint8_t layer, const struct behavior_auto_layer_local_config *config,
                          struct active_auto_layer_local **auto_layer) {
    for (int i = 0; i < ZMK_BHV_AUTO_LAYER_LOCAL_MAX_ACTIVE; i++) {
        struct active_auto_layer_local *const ref_auto_layer = &active_auto_layers[i];
        if (!ref_auto_layer->is_active) {
            ref_auto_layer->is_active = true;
            ref_auto_layer->layer = layer;
            ref_auto_layer->config = config;
            ref_auto_layer->keypresses_remaining = config->max_keypresses;
...
static void clear_auto_layer(struct active_auto_layer_local *auto_layer) {
    auto_layer->is_active = false;
}

static void activate_auto_layer(struct active_auto_layer_local *auto_layer) {
...
static void deactivate_auto_layer(struct active_auto_layer_local *auto_layer) {
...
static int on_auto_layer_binding_pressed(struct zmk_behavior_binding *binding,
    ...
    const struct behavior_auto_layer_local_config *cfg = dev->config;

    struct active_auto_layer_local *auto_layer = find_auto_layers(binding->param1);
    if (auto_layer == NULL) {
...
static int auto_layer_local_keycode_state_changed_listener(const zmk_event_t *eh);

ZMK_LISTENER(behavior_auto_layer_local, auto_layer_local_keycode_state_changed_listener);
ZMK_SUBSCRIPTION(behavior_auto_layer_local, zmk_keycode_state_changed);
...
static bool auto_layer_should_continue(const struct behavior_auto_layer_local_config *config,
...
static int auto_layer_local_keycode_state_changed_listener(const zmk_event_t *eh) {
    ...
    for (int i = 0; i < ZMK_BHV_AUTO_LAYER_LOCAL_MAX_ACTIVE; i++) {
        struct active_auto_layer_local *auto_layer = &active_auto_layers[i];
        ...
        if (auto_layer->keypresses_remaining == 0) {
            continue;
        }

        auto_layer->keypresses_remaining--;
        if (auto_layer->keypresses_remaining == 0) {
            LOG_DBG("Deactivating auto_layer after max keypresses");
            deactivate_auto_layer(auto_layer);
        }
    }
...
static int behavior_auto_layer_local_init(const struct device *dev) {
...
#define AL_LOCAL_INST(n)                                                                           \
    static const struct key_list auto_layer_continue_list_##n = PROP_KEY_LIST(n, continue_list);   \
    static struct behavior_auto_layer_local_config behavior_auto_layer_local_config_##n = {        \
        ...
        .max_keypresses = DT_INST_PROP_OR(n, max_keypresses, 0),                                   \
    };                                                                                             \
    BEHAVIOR_DT_INST_DEFINE(n, behavior_auto_layer_local_init, NULL, NULL,                         \
                            &behavior_auto_layer_local_config_##n, POST_KERNEL,                    \
                            CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, &behavior_auto_layer_driver_api);

DT_INST_FOREACH_STATUS_OKAY(AL_LOCAL_INST)
